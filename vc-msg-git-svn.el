;;; vc-msg-git-svn.el --- extract git commit message
...
(defcustom vc-msg-git-svn-program "git"
  "Git program."
  :type 'string
  :group 'vc-msg)

(defcustom vc-msg-git-svn-show-commit-function nil
  "The function to show git commit.  Revision is the parameter of the function.
It can be set the value like `magit-show-commit'."
  :type 'function
  :group 'vc-msg)

(defun vc-msg-git-svn-shell-output (cmd)
  "Generate clean output by running CMD in shell."
  (let* ((default-directory (vc-msg-sdk-git-rootdir)))
    (string-trim (shell-command-to-string cmd))))

(defun vc-msg-git-svn-generate-cmd (opts)
  "Generate Git command from OPTS."
  (format "%s --no-pager %s" vc-msg-git-svn-program opts))

(defun vc-msg-git-svn-parse-blame-output (output)
...
;;;###autoload
(defun vc-msg-git-svn-blame-arguments (line-num)
...
(defmacro vc-msg-git-svn-valid-p (output)
  "OUTPUT is valid."
  `(string-match-p "^author " ,output))

(defun vc-msg-git-svn-find-blame-cmd-recursively (command info str common-opts)
...
COMMON-OPTS is used to build new blame command."
  (let* ((id (plist-get info :id))
         ...
         (next-cmd (vc-msg-git-svn-generate-cmd (format "%s %s^ -- %s"
                                                    common-opts
                                                    id
                                                    file)))

         (next-output (vc-msg-git-svn-shell-output next-cmd))
         next-info)
    (cond
     ((not (and (vc-msg-git-svn-valid-p next-output)
                (setq next-info (vc-msg-git-svn-parse-blame-output next-output))
                (string-match-p (regexp-quote str)
                                (plist-get next-info :code))))
      command)
     (t
      (vc-msg-git-svn-find-blame-cmd-recursively next-cmd
                                             next-info
                                             str
                                             common-opts)))))

(defun vc-msg-git-svn-generate-blame-cmd (file line-num version)
  "Generate git blame command from FILE, LINE-NUM and VERSION."
  (let* ((str (vc-msg-sdk-selected-string))
         output
         info
         (common-opts (vc-msg-git-svn-blame-arguments line-num))
         (cmd (vc-msg-git-svn-generate-cmd (format "%s %s -- %s"
                                               common-opts
                                               version
                                               file))))
    (cond
     ((not str)
      cmd)
     ((vc-msg-git-svn-valid-p (setq output (vc-msg-git-svn-shell-output cmd)))
      (setq info (vc-msg-git-svn-parse-blame-output output))
      (vc-msg-git-svn-find-blame-cmd-recursively cmd
                                             info
                                             (string-trim str)
                                             common-opts)))))

;;;###autoload
(defun vc-msg-git-svn-execute (file line-num version)
...
'(:id str :author str :author-time str :summary str)."
  ...
  (let* ((cmd (vc-msg-git-svn-generate-blame-cmd file line-num version))
         (output (vc-msg-git-svn-shell-output cmd)))

    ;; I prefer simpler code:
    ;; if output doesn't match certain text pattern, the command fails
    (cond
     ((vc-msg-git-svn-valid-p output)
      (vc-msg-git-svn-parse-blame-output output))
     (t
      (format "`%s` failed." cmd)))))

;;;###autoload

(defun vc-msg-git-svn-format (info)
  ...
  (let* ((author (plist-get info :author)))
	(cond
	 ((string-match-p "Not Committed Yet" author)
	  "* Not Committed Yet*")
	 (t
	  (format "Commit: %s ==> svn rev: %s\nAuthor: %s\nDate: %s\nTimezone: %s\n\n%s"
			  (vc-msg-sdk-short-id (plist-get info :id))
			  (shell-command-to-string (format "git svn find-rev %s" (plist-get info :id)))
			  author
...
(defun vc-msg-git-svn-svn-format (info)
  "Format the message for popup from INFO."
  (let* ((author (plist-get info :author)))
	(cond
	 ((string-match-p "Not Committed Yet" author)
...
(defun vc-msg-git-svn-show-code ()
  "Show code."
  (let* ((info vc-msg-previous-commit-info)
         (id (plist-get info :id)))
    (cond
     (vc-msg-git-svn-show-commit-function
      (funcall vc-msg-git-svn-show-commit-function id))
     (t
      ;; show one commit information
      (vc-msg-sdk-get-or-create-buffer
       "vs-msg"
       (vc-msg-git-svn-shell-output (vc-msg-git-svn-generate-cmd (format "show %s" id))))))))

(defun vc-msg-git-svn-copy-info (field)
  "Copy info of FIELD."
  (let* ((id (plist-get vc-msg-previous-commit-info field)))
    (kill-new id)
    (message "%s => kill-ring" id)))

(defcustom vc-msg-git-svn-extra
  '(("c" "[c]ode" vc-msg-git-svn-show-code)
    ("h" "[h]ash" (lambda () (vc-msg-git-svn-copy-info :id)))
    ("a" "[a]uthor" (lambda () (vc-msg-git-svn-copy-info :author))))
...
(defun vc-msg-git-svn-extra-setup ()
  "Set up `vc-msg-git-svn-extra' for more functionalities."
...
(vc-msg-git-svn-extra-setup)

(provide 'vc-msg-git-svn)
;;; vc-msg-git-svn.el ends here
